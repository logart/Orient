...
public class OSafeBinaryConverter implements OBinaryConverter {
  ...
  public int getInt(byte[] buffer, int pointer, int offset) {
	...
	private byte[] short2bytes(final short value, final byte[] b, final int iBeginOffset) {
		b[iBeginOffset] = (byte) ((value >>> 8) & 0xFF);
		b[iBeginOffset + 1] = (byte) ((value >>> 0) & 0xFF);
		return b;
	}

	private short bytes2short(final byte[] b, final int offset) {
		return (short) ((b[offset] << 8) | (b[offset + 1] & 0xff));
	}

	private int bytes2int(final byte[] b, final int offset) {
		return (b[offset]) << 24 | (0xff & b[offset + 1]) << 16 | (0xff & b[offset + 2]) << 8 | ((0xff & b[offset + 3]));
	}

	private byte[] int2bytes(final int value, final byte[] b, final int iBeginOffset) {
	...
	private byte[] long2bytes(final long value, final byte[] b, final int iBeginOffset) {
	...
	private long bytes2long(final byte[] b, final int offset) {
